...
* `_terminal` is the [`IJBPaymentTerminal`](/dev/api/v2/interfaces/ijbpaymentterminal.md) from which distributions are being limited.
...
* The function returns:
  ...
  * `distributionLimitCurrency` is the currency from [`JBCurrencies`](/dev/api/v2/libraries/jbcurrencies.md) that the returned distribution limit is in terms of.

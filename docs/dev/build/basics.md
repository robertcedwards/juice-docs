...
Launching a project will mint a new NFT in the [`JBProjects`](/dev/api/v2/contracts/jbprojects/README.md) contract. The owner can be found using [`JBProjects.ownerOf(...)`](https://docs.openzeppelin.com/contracts/4.x/api/token/erc721#IERC721-ownerOf-uint256-).

```
function ownerOf(uint256 _projectId) external returns (address owner) { ... }
```

The project's metadata can be found using [`JBProjects.metadataContentOf(...)`](/dev/api/v2/contracts/jbprojects/properties/metadatacontentof.md).
...
Funding cycle data can be found in the [`JBFundingCycleStore`](/dev/api/v2/contracts/jbfundingcyclestore/README.md) contract. A funding cycle configuration can be found using [`JBFundingCycleStore.get(...)`](/dev/api/v2/contracts/jbfundingcyclestore/read/get.md), where `_configuration` is the block timestamp when the funding cycle was configured, or using [`JBController.getFundingCycleOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/getfundingcycleof.md) if the funding cycle's metadata is needed alongside.
...
The project's queued funding cycle can be found using [`JBFundingCycleStore.queuedOf(...)`](/dev/api/v2/contracts/jbfundingcyclestore/read/queuedof.md), or using [`JBController.queuedFundingCycleOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/queuedfundingcycleof.md) if the funding cycle's metadata is needed alongside.
...
The project's latest configured funding cycle can be found using [`JBFundingCycleStore.latestConfiguredOf(...)`](/dev/api/v2/contracts/jbfundingcyclestore/read/latestconfiguredof.md), or using [`JBController.latestConfiguredFundingCycleOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/latestconfiguredfundingcycleof.md) if the funding cycle's metadata is needed alongside. These calls also return the current ballot status for the configuration.
...
A project's splits data can be found in the [`JBSplitStore`](/dev/api/v2/contracts/jbsplitsstore/README.md) contract. A group of splits belonging to any particular group during any particular funding cycle configuration can be found using [`JBSplitStore.splitsOf(...)`](/dev/api/v2/contracts/jbsplitsstore/read/splitsof.md). The funding cycle's configuration is used as the `_domain` within which the splits apply.
...
Constraints on accessing a project's funds can found in the [`JBController`](/dev/api/v2/contracts/or-controllers/jbcontroller) contract used to launch the project. The distribution limit of any payment terminal during any funding cycle configuration can be found using [`JBController.distributionLimitOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/distributionlimitof.md). The currency being used for this distribution limit is returned alongside.
...
The overflow allowance from any payment terminal during any funding cycle configuration can be found using [`JBController.overflowAllowanceOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/overflowallowanceof.md). The currency being used for this overflow allowance is returned alongside. 
...
The [`JBDirectory`](/dev/api/v2/contracts/jbdirectory/README.md) contract stores addresses of payment terminals that a project is currently accepting funds through. A project's currently set terminals can be found using [`JBDirectory.terminalsOf(...)`](/dev/api/v2/contracts/jbdirectory/read/terminalsof.md).

```
function terminalsOf(uint256 _projectId) external view override returns (IJBPaymentTerminal[] memory) { ... }
```

If a project has multiple terminals for the same token, the primary terminal that it wishes to accept that token type through can be found using [`JBDirectory.primaryTerminalOf(...)`](/dev/api/v2/contracts/jbdirectory/read/primaryterminalof.md).
...
The [`JBDirectory`](/dev/api/v2/contracts/jbdirectory/README.md) contract also stores the address of the controller that is managing a project's funding cycles and tokens. A projects current terminal can be found using [`JBDirectory.controllerOf(...)`](/dev/api/v2/contracts/jbdirectory/properties/controllerof.md).
...
In payment terminals based on the [`JBPayoutRedemptionPaymentTerminal`](/dev/api/v2/contracts/or-payment-terminals/or-abstract/jbpayoutredemptionpaymentterminal), such as [`JBETHPaymentTerminal`](/dev/api/v2/contracts/or-payment-terminals/jbethpaymentterminal/README.md)'s and [`JBERC20PaymentTerminal`](/dev/api/v2/contracts/or-payment-terminals/jberc20paymentterminal/README.md)'s, a project's treasury balance can be found in its store contract. For example, in the [`JBSingleTokenPaymentTerminalStore`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/README.md), the balance can be found using [`JBSingleTokenPaymentTerminalStore.balanceOf(...)`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/properties/balanceof.md).
...
The project's current overflow for a terminal can also be found in the store contracts. For example, in the [`JBSingleTokenPaymentTerminalStore`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore), the terminal's overflow can be found using [`JBSingleTokenPaymentTerminalStore.currentOverflowOf(...)`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/read/currentoverflowof.md).
...
A terminal store can also resolve the total amount of overflow in all of a project's terminals. For example, in the [`JBSingleTokenPaymentTerminalStore`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore), the project's overall overflow can be found using [`JBSingleTokenPaymentTerminalStore.currentTotalOverflowOf(...)`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/read/currenttotaloverflowof.md). You will need to send the number of decimals you're expecting the returned fixed point number to include, and the currency it is in terms of.
...
Each holder's balance of a project's token can be found in the [`JBTokenStore`](/dev/api/v2/contracts/jbtokenstore) contract. The balance can be found using [`JBTokenStore.balanceOf(...)`](/dev/api/v2/contracts/jbtokenstore/read/balanceof.md).

```
function balanceOf(address _holder, uint256 _projectId) external view returns (uint256 _result) { ... }
```

The project token's total supply can also be found in the [`JBTokenStore`](/dev/api/v2/contracts/jbtokenstore) contract using [`JBTokenStore.totalSupplyOf(...)`](/dev/api/v2/contracts/jbtokenstore/read/totalsupplyof.md)
...
A project's undistributed reserved token balance can be found in the project's current controller. For example in the [`JBController`](/dev/api/v2/contracts/or-controllers/jbcontroller/README.md), this balance can be found using [`JBController.reservedTokenBalanceOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/reservedtokenbalanceof.md).
...
For projects using [`JBController`](/dev/api/v2/contracts/or-controller/jbcontroller), the project token's total supply including any allocated reserved tokens that have yet to be distributed can be found in using [`JBController.totalOutstandingTokensOf(...)`](/dev/api/v2/contracts/or-controllers/jbcontroller/read/totaloutstandingtokensof.md). 
...
Any distribution limit used by a project can be found in the terminal store contract for each terminal. For example, in the [`JBSingleTokenPaymentTerminalStore`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/README.md), the distribution limit used during a funding cycle can be found by calling [`JBSingleTokenPaymentTerminalStore.usedDistributionLimitOf(...)`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/properties/useddistributionlimitof.md).
...
Any overflow allowance used can also be found in the terminal store contracts for each terminal. For example, in the [`JBSingleTokenPaymentTerminalStore`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/README.md), the overflow allowance used during a funding cycle can be found using [`JBSingleTokenPaymentTerminalStore.usedOverflowAllowanceOf(...)`](/dev/api/v2/contracts/jbsingletokenpaymentterminalstore/properties/usedoverflowallowanceof.md).
...
The same price feeds the protocol uses internally can be accessed externally through the [`JBPrices`](/dev/api/v2/contracts/jbprices/README.md) contract using [`JBPrices.priceFor(...)`](/dev/api/v2/contracts/jbprices/read/pricefor.md). This will revert if a feed is not found for the currency pair provided.
...
Reconfigurations are subject to the approval of the ballot contract included in the current funding cycle. The current ballot state can be found using [`JBFundingCycleStore.ballotStateOf(...)`](/dev/api/v2/contracts/jbfundingcyclestore/read/currentballotstateof.md).
...
The token currently being used by a project can be found in the [`JBTokensStore`](/dev/api/v2/contracts/jbtokenstore/README.md) contract by using [`JBTokenStore.tokenOf(...)`](/dev/api/v2/contracts/jbtokenstore/properties/tokenof.md). This will return a zero address if the project hasn't yet issued tokens or changed into a custom token. 

```
function tokenOf(uint256 _projectId) external view override returns (IJBToken) { ... }
```

The project a token is currently being used for can be found by calling [`JBTokenStore.projectOf(...)`](/dev/api/v2/contracts/jbtokenstore/properties/projectof.md).
...
Each project token holder's unclaimed balance can be found in the [`JBTokensStore`](/dev/api/v2/contracts/jbtokenstore/README.md) contract using [`JBTokenStore.unclaimedBalanceOf(...)`](/dev/api/v2/contracts/jbtokenstore/properties/unclaimedbalanceof.md).

```
function unclaimedBalanceOf(address _holder, uint256 _projectId) external view override returns (uint256) { ... }
```

A project's total unclaimed token supply can be found using [`JBTokenStore.unclaimedTotalSupplyOf(...)`](/dev/api/v2/contracts/jbtokenstore/properties/unclaimedtotalsupplyof.md)

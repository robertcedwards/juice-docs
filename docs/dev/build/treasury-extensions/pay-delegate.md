...
A contract can become a treasury pay delegate by adhering to [`IJBPayDelegate`](/dev/api/v2/interfaces/ijbpaydelegate.md):
...
When extending pay functionality with a delegate, the protocol will pass a [`JBDidPayData`](/dev/api/v2/data-structures/jbdidpaydata.md) to the `didPay(...)` function:
